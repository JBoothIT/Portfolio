...
KEY_NAME = 'CS403AlphaVPNKey'
secGrpName = 'CS403AlphaVPN'
instanceIDList = [] # list of all the server instance IDs created
locationLastHop = '' # to capture the Region ID
...
listOfCloneRegionAMIs = {
    ...
    'eu-west-2' : 'ami-0d4450c0945052310',  #EU (London)
...
def printStatusMessage(message, start_time):
    print(message + " --- %s seconds ---" % (time.time() - start_time))
    
    
def getLocationID(argument):
    ...
    return listOSupportedRegions.get(locArgument, "invalid selection")
...
def getCloneAMI(argument):
    ...
    return listOfCloneRegionAMIs.get(cloneArgument, "invalid selection")

def CreateInstance(ec2Resource, imageID, key_name, securityGroupID):
...
def writeOPVNViaOS(myOVPNIP):
    ...
    # connect to VPN
    shellSTR='cd c:\\Program Files\\OpenVPN\\bin && openvpn-gui.exe --connect "client101.ovpn"'
    os.system(shellSTR)
    #os.system('openvpn-gui.exe --connect "client101.ovpn"')
    
    return
    
# ============= call this function to get it started ==========
def startVPN(location, NumHops):

    # set up variables, convert the plain english location to a Region ID
    locationLastHop = listOSupportedRegions.get(location, "invalid selection")
    myNumHops = int(NumHops)
    ...
    importKeys(ec2cNR)
    printStatusMessage('Keys are good', start_time)
    
    # complete pre-reqs -- configure security groups
    securityGrpID = setSecurityGroup(ec2cNR, secGrpName)
    printStatusMessage('Security Groups are good', start_time)
    ...
    #myInstanceID = (CreateInstance(ec2NR, imageIDConditional, KEY_NAME, secGrpName))
    myInstanceID = (CreateInstance(ec2NR, imageIDConditional, KEY_NAME, securityGrpID))
    myOVPNInstance = ec2NR.Instance(myInstanceID)
    myOVPNInstance.wait_until_running()
    myOVPNIP = myOVPNInstance.public_ip_address
    printStatusMessage('VPN instance is up', start_time)
    print(myOVPNIP)
    configureVPNServerViaOS(myOVPNIP)
    printStatusMessage('VPN server configured', start_time)
    
    # add instance ID to our list, so we can delete it later
    instanceIDList.append(myInstanceID)
    
    if  myNumHops == 1: # if the user only asked for the VPN server, write the ovpn client config file and start
        writeOPVNViaOS(myOVPNIP)
        os.system('openvpn-gui.exe --connect "client101.ovpn"')
        return
    
    # if more than one hop, start creating a port forwarding VM, forwarding to the OpenVPN VM, rinse and repeat
    l = 2 # just a counter for the while loop
    lastServerIP = myOVPNIP # for the port forwarding scheme    
    while l <= myNumHops:
        myInstanceID2 = (CreateInstance(ec2NR, imageIDConditional, KEY_NAME, secGrpName))
        myHopInstance = ec2NR.Instance(myInstanceID2)
        myHopInstance.wait_until_running()
        myHopIP = myHopInstance.public_ip_address
        printStatusMessage('Hop' + str(l) +' server is up', start_time)
        print('=======\nHop Server :',myInstanceID2, myHopIP)
        configureHopServerViaOS(lastServerIP, myHopIP)
        printStatusMessage('Hop server is configured', start_time)
        instanceIDList.append(myInstanceID2)
        
        # if this is the last hop server, write the ovpn client config file
        if l == myNumHops:
        ...
        # otherwise reset/record variables, rinse and repeat
        lastServerIP = myHopIP
        l = l + 1
        #instanceIDList.append(myInstanceID2)
    
    return

# ========== call this function to delete the instances =======
def deleteVPNs(location):
    # from Robert's code
    ...
    ec2NR.instances.filter(InstanceIds = instanceIDList).terminate()
    print(str(instanceIDList))
    printStatusMessage('waiting for termination...', start_time)  
    for inst in instanceIDList:
        thisInstance = ec2NR.Instance(inst)
        thisInstance.wait_until_terminated()
        printStatusMessage('ID: ' + str(inst) + ' terminated...', start_time)
        
    # delete keys
    response = ec2cNR.delete_key_pair(
                                    KeyName=KEY_NAME,
                                    )
    printStatusMessage('Keys deleted...', start_time)
        
    # delete security groups
    # first check to ensure security group is not in use
    '''
    secGrpInUse = True
    while secGrpInUse == True:
        instances_dict = ec2cNR.describe_instances()
        reservations = instances_dict['Reservations']
        for i in reservations:
            for j in i['Instances']:
                for k in j['SecurityGroups']:
                    if (k['GroupName']) == secGrpName:
                        printStatusMessage('Security Groups still in use, waiting...', start_time)
                        time.sleep(20)
                        #print('got to the right place')
                        # set marker to True (still used)
                        secGrpInUse = True
                    else:
                        secGrpInUse = False
    
                        response = ec2cNR.delete_security_group(GroupName=secGrpName)
    printStatusMessage('Security Group deleted...', start_time)
    '''
    ...
    for inst in instanceIDList:
        ...
        thisInstance.wait_until_terminated()
    response = ec2cNR.delete_security_group(GroupName=secGrpName)
    printStatusMessage('Security Groups deleted...', start_time)

    print("done.  Thanks for practicing safe networking with Team Alpha!")   
    return
