...
print (ipaddressH)
time.sleep(15)
shellSTR='cd c:\\tempOVPN && ssh -i virgkey.pem -o "StrictHostKeyChecking=no" ubuntu@'+ipaddressH + ' "sudo sysctl net.ipv4.ip_forward=1; sudo iptables -t nat -A PREROUTING -p udp --dport 1194 -j DNAT --to-destination '+ipaddress+':1194; sudo iptables -t nat -A POSTROUTING -j MASQUERADE"'
os.system(shellSTR)
time.sleep(15)
